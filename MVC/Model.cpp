...
#include <memory> si::mvc::Model::Model()
{
        ...
        player = std::make_shared<si::entity::Player>(
                size, position, 1, entity::collideObjectType::friendly, 1);

        entity::Size sizeSC{2.4, 0.25};
        entity::Position positionSC{-2.85f, 3.85f};
        scoreCounter = std::make_shared<si::entity::Counter>(
                sizeSC, positionSC, 0, entity::counterType::score);

        entity::Size sizeCC{2.4, 0.25};
        entity::Position positionCC{-2.85f, 3.5f};
        coolDownCounter = std::make_shared<si::entity::Counter>(
                sizeCC, positionCC, 0, entity::counterType::cooldown);

        entity::Size sizeLC{2, 0.25};
        entity::Position positionLC{0.65f, 3.85f};
        healthCounter = std::make_shared<si::entity::Counter>(
                sizeLC, positionLC, 3, entity::counterType::health);

        entity::Size sizeSS{6, 2};
        entity::Position positionSS{-3, 6};
        sky = std::make_shared<si::entity::Immortal>(
                sizeSS, positionSS, 1, entity::collideObjectType::hostile);

        entity::Size sizeES{6, 2};
        entity::Position positionES{-3, -4};
        earth = std::make_shared<si::entity::Immortal>(
                sizeES, positionES, 1, entity::collideObjectType::friendly);
}
